...
import com.qixiu.lejia.databinding.ActWaterPaidBinding;
import com.qixiu.lejia.databinding.ActWaterUnpaidBinding;
 ...
 * </pre>
 */
public class WaterCostDetailAct extends BaseServicePayAct implements WECostDetailContract.View {
    ...
    public static void start(Activity context, @Nullable String billId, int payStatus, int requestCode) {
        Intent starter = new Intent(context, WaterCostDetailAct.class);
    ...
    protected View onCreateContentView(LayoutInflater inflater) {
        ...
        if (mExtraPayStatus == 1) {
            mBinding = ActWaterPaidBinding.inflate(inflater);
            ...
            mBinding = ActWaterUnpaidBinding.inflate(inflater);
            View rootView= mBinding.getRoot();
            relativeLayout_discount=rootView.findViewById(R.id.relative_discount);
            relativeLateFee = rootView.findViewById(R.id.relativeLateFee);
            ...
            mBinding.setVariable(BR.actionHandler, (ItemActionHandler<Object>) o -> {
                ...
                if (scene == 0) {
                    //选择支付方式
                    ActWaterUnpaidBinding b = (ActWaterUnpaidBinding) mBinding;
    ...
    private void doPay() {
        ActWaterUnpaidBinding binding = (ActWaterUnpaidBinding) mBinding;
        btn_pay = mBinding.getRoot().findViewById(R.id.btn_pay);
        btn_pay.setEnabled(false);
        WECostDetail item = binding.getResp();
        if (item == null) {
            return;
        }
        startPay(2, item.getRoomId(), item.getSumAll(), null, mExtraBillId);
        //startPay(2, item.getRoomId(), "0.01", null, mExtraBillId);
    }
